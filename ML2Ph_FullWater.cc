...
G4Material *mangoTissue = new G4Material ("SoftTissue", 1.00*g/cm3, 
                                             numberofElements = 8);
 ...
 fullWaterPhantomPV = new G4PVPlacement(0, centre, "fullWaterPhantomPV", fullWaterPhantomLV, PVWorld, false, 0);
  ...
  //G4VSolid* solVoxel = new G4Orb(xRepName,sensSize.x()/4);
   G4VSolid*  solVoxel = new G4Ellipsoid(xRepName,sensSize.x()/4,sensSize.x()/2,sensSize.x()/4);

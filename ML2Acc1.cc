...
bool CML2Acc1::screen()                    ///modificado
{
	bool bCreated = false;
	switch (idEnergy)
	{
		case 6:
			...
			//------------------------target 6MV------------------------
			G4double screenDim_x = 50.0*cm;
			G4double screenDim_y = 50.0*cm;
			G4double screenDim_z = 1.0*mm;
			G4Box* screen_box = new G4Box("screen_box",screenDim_x,screenDim_y,screenDim_z);
			G4LogicalVolume *screen_log = new G4LogicalVolume(screen_box,Vac,"screen_log",0,0,0);
			G4double screenPos_x = 0.0*m;
			G4double screenPos_y = 0.0*m;
			G4double screenPos_z = 0.5*m;
			new G4PVPlacement(0,
					G4ThreeVector(screenPos_x,screenPos_y,screenPos_z),
					"screen",screen_log,PVWorld,false,0);
               ///////////////////////////////////////////////////////////////
			//G4double screenBDim_x = 30.0*cm;
			//G4double screenBDim_y = 30.0*cm;
			//G4double screenBDim_z = 0.1*cm;
			//G4Box* screenB_box = new G4Box("screenB_box",screenBDim_x,screenBDim_y,screenBDim_z);
			//G4LogicalVolume *screenB_log = new G4LogicalVolume(screenB_box,Vac,"screenB_log",0,0,0);
			//G4double screenBPos_x = 0.0*m;
			//G4double screenBPos_y = 0.0*m;
			//G4double screenBPos_z = 0.6*m;
			//new G4PVPlacement(0,
			//		G4ThreeVector(screenBPos_x,screenBPos_y,screenBPos_z),
			//		"screenB",screenB_log,PVWorld,false,0);

			// ***********  REGIONS for CUTS
			G4Region *regVol;
			regVol= new G4Region("screen");
			G4ProductionCuts* cuts = new G4ProductionCuts;
			cuts->SetProductionCut(0.1*cm);
			regVol->SetProductionCuts(cuts);

			screen_log -> SetRegion(regVol);
			regVol -> AddRootLogicalVolume(screen_log);
			//screenB_log -> SetRegion(regVol);
			//regVol -> AddRootLogicalVolume(screenB_log);
			...
			screen_log -> SetVisAttributes(simpleWSVisAtt);
			//screenB_log -> SetVisAttributes(simpleCuSVisAtt);
			
			MySensitiveDetector *sensDet = new MySensitiveDetector("SensitiveDetector");
			screen_log->SetSensitiveDetector(sensDet);
...
bool CML2Acc1::MLC() //MultiLeaf Collimator
{
	...
	centre = centreStart + G4ThreeVector(-2*nhalfLeaves*boxSize.getX(), 0.,0.);

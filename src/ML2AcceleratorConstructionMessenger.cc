...
CML2AcceleratorConstructionMessenger::CML2AcceleratorConstructionMessenger(CML2AcceleratorConstruction *acceleratorConstructor) : pAcceleratorConstructor (acceleratorConstructor)
{
	...
	bfieldm = new G4UIcmdWithADoubleAndUnit("/field/value", this);
	bfieldm -> SetDefaultUnit("tesla");
	bfieldm -> SetDefaultValue(1.);
	bfieldm -> SetGuidance("campo magnetico distance between the isocentre and the target of the accelerator");
	//pAcceleratorConstructor->MagneticField::SetField(1*tesla);
	 //pAcceleratorConstructor->ConstructSDandField();

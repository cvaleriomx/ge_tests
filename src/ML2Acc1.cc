...
bool CML2Acc1::target()
{
	...
	switch (idEnergy)
	{
		case 6:
			...
			//------------------------target 6MV------------------------
			G4double targetz0 = 60.5;
			G4double targetADim_x = 27.5*cm;
			G4double targetADim_y = 50.0*cm;
			...
			G4double targetAPos_z = (0.20055+targetz0)*cm ;
			new G4PVPlacement(0,
					G4ThreeVector(targetAPos_x,targetAPos_y,targetAPos_z),
					"targetA",targetA_log,PVWorld,false,0);

			G4double targetBDim_x = 27.5*cm;
			G4double targetBDim_y = 50.0*cm;
			...
			G4double targetBPos_z = (0.07736+targetz0)*cm ;
			new G4PVPlacement(0,
					G4ThreeVector(targetBPos_x,targetBPos_y,targetBPos_z),
					"targetB",targetB_log,PVWorld,false,0);
					
			G4double targetCDim_x = 27.5*cm;
			G4double targetCDim_y = 50.0*cm;
			G4double targetCDim_z = 0.1*cm;
			G4Box* targetC_box = new G4Box("targetC_box",targetCDim_x,targetCDim_y,targetCDim_z);
			G4LogicalVolume *targetC_log = new G4LogicalVolume(targetC_box,WATER,"targetC_log",0,0,0);
			G4double targetCPos_x = 0.0*m;
			G4double targetCPos_y = 0.0*m;
			G4double targetCPos_z = (0.9+targetz0)*cm ;
			new G4PVPlacement(0,
					G4ThreeVector(targetCPos_x,targetCPos_y,targetCPos_z),
					"targetC",targetC_log,PVWorld,false,0);
			...
			targetC_log -> SetRegion(regVol);
			regVol -> AddRootLogicalVolume(targetC_log);

			// Visualization attributes
			G4VisAttributes* simpleWSVisAtt, *simpleCuSVisAtt,*simpleWaSVisAtt;;
			...
			simpleWaSVisAtt = new G4VisAttributes(G4Colour::Blue());
			simpleWaSVisAtt -> SetVisibility(true);
			targetA_log -> SetVisAttributes(simpleWSVisAtt);
			targetB_log -> SetVisAttributes(simpleCuSVisAtt);
			targetC_log -> SetVisAttributes(simpleWaSVisAtt);
...
bool CML2Acc1::CONE() //MultiLeaf Collimator
{
	bool bCreated = true;
   ...
   G4Material *steel = new G4Material("steel", 7.76 *g/cm3, 4);
   steel ->AddMaterial(Fe, 0.935);
   steel ->AddMaterial(S, 0.01);
   steel ->AddMaterial(Mn, 0.05);
   steel ->AddMaterial(C, 0.005);
    //creando cono1
    G4double pdx1 = 1.*cm;
    G4double pdx2 = 22.5*cm;
    G4double pdy1 = 1.*cm;
    G4double pdy2 = 45.*cm;
    G4double pdz = 120.*cm;
    ...
    //creando cono2
    G4double pd2x1 = 6.*cm;
    G4double pd2x2 = 27.5*cm;
    G4double pd2y1 = 6.*cm;
    G4double pd2y2 = 50.*cm;
    G4double pd2z = 120.*cm;
    ...
    G4LogicalVolume * ConeLV = new G4LogicalVolume(subtract, steel, "ConeLV", 0, 0, 0);
    ...
    //G4VPhysicalVolume *Cone = new G4PVPlacement(0, G4ThreeVector(0.0,0.0,148.*cm), "ConePV", ConeLV,PVWorld, false, 0);
    G4VPhysicalVolume *Cone = new G4PVPlacement(0, G4ThreeVector(0.0,0.0,-60.*cm), "ConePV", ConeLV,PVWorld, false, 0);

...
void CML2PrimaryGenerationAction::GenerateFromRandom()
{
	...
	bool Mfd =  false;
		if (Mfd)
		{
			...
			pos.setX(rho*std::sin(alpha));
			pos.setY(rho*std::cos(alpha));
			pos.setZ(-1.5*cm); // the primary electrons are generated 5 mm before the target
		}
		else 
		{
		double xlargo=22.5;
		double ylargo= 45.;
		G4double posx = G4UniformRand()*2*xlargo -1*xlargo;
		G4double posy = G4UniformRand()*2*ylargo -1*ylargo;
			pos.setX(posx*cm);
			pos.setY(posy*cm);
			pos.setZ(-41.3*cm);
			//pos.setZ(-(accTargetZPosition +5.)*mm); // the primary electrons are generated 5 mm before the target
		        G4double Thetar=atan(posy/240);
			sinTheta = sin(Thetar);
			cosTheta = std::sqrt(1 - sinTheta*sinTheta);
			phi = twopi*G4UniformRand();
			dir.set(sinTheta*std::cos(phi), sinTheta*std::sin(phi), cosTheta);
